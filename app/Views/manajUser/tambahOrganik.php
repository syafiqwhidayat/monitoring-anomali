...
<div class="container-xl">
    <div class="page-header d-print-none mb-3">
        <div class="row g-2 align-items-center">
            <div class="col">
                <h2 class="page-title">Tambah User Organik</h2>
                <div class="text-muted mt-1">Daftarkan petugas baru ke dalam sistem monitoring</div>
    ...
    <div class=" row row-cards">
        <div class="col-md-8">
            <form action="<?= base_url('/user/tambah-organik/store'); ?>" method="post" class="card shadow-sm" style="border-radius: 12px;">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" name="email" class="form-control <?= session('errors.email') ? 'is-invalid' : '' ?>"
                                placeholder="contoh@bps.go.id" value="<?= old('email') ?>" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control <?= session('errors.name') ? 'is-invalid' : '' ?>"
                                placeholder="Masukkan nama tanpa gelar" value="<?= old('name') ?>" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="wilayah_kerja" class="form-select <?= session('errors.wilayah_kerja') ? 'is-invalid' : '' ?>"
                                ...
                                <?php foreach ($wilayah_sumbar as $kode => $nama) : ?>
                                    <option value="<?= $kode; ?>" <?= (old('wilayah_kerja') == $kode) ? "selected" : "" ?>><?= $nama; ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="role" class="form-select <?= session('errors.role') ? 'is-invalid' : '' ?>" required>
                                <?php foreach ($roles as $kode => $nama) : ?>
                                    <option value="<?= $kode; ?>" <?= (old('role') == $kode) ? "selected" : "" ?>><?= $nama; ?></option>
                                <?php endforeach; ?>
                            </select>
